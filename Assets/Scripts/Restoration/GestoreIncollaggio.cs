...
public class GestoreIncollaggio : MonoBehaviour, IRestorationPhaseManager
{
    ...
    private void IniziaIncollaggio()
    {
        ...
        if (tavoloCorrente == null || tavoloCorrente.vaschettaCorrente == null)
        {
            Debug.LogError("Tavolo di lavoro o vaschetta corrente non validi.");
            return;
        }
        ...
        // Recupero dell'anfora assemblata persistente dal tavolo di lavoro
        anforaAssemblata = tavoloCorrente.anforaAssemblata;
        if (anforaAssemblata == null)
        {
            Debug.LogError("Anfora assemblata non trovata nel tavolo di lavoro.");
            return;
        }
        ...
        if (vaschettaGO == null)
        {
            Debug.LogError("GameObject della vaschetta non trovato nel tavolo di lavoro.");
            return;
        }
        ...
        if (config == null)
        {
            Debug.LogError($"Componente ConfigurazioneVaschetta mancante sulla vaschetta {vaschettaGO.name}.");
            return;
        }
        ...
        if (Mouse.current.leftButton.isPressed)
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit hit, Mathf.Infinity, layerRestauro))
            {
                ...
                else
                {
                    ...
                    Debug.LogWarning($"Il raycast ha colpito l'oggetto {hitName} che non fa parte dell'anfora assemblata.");
                }
            }
            else
            {
    ...
    private System.Collections.IEnumerator SequenzaCompletamento()
    {
        ...
        if (prefabIntero != null && anforaAssemblata != null)
        {
            ...
            // Distrugge il vecchio contenitore vaschettaGameObject del tavolo e imposta l'anfora intera
            if (tavoloCorrente != null)
            {
        ...
        {
            Debug.LogWarning("Prefab dell'anfora intera non assegnato in VaschettaSO.");
        }
